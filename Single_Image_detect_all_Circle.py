...
def Single_Image_Detect_ThreeCicle(img):
    ...
    if (Yellow_hierarchy.any() == None):
        Yellow_con_fill = np.zeros(img.shape)
    else:
    ...
    return img_contours, Red_contours_fill, Yellow_con_fill, Blue_con_fill



def Single_Image_Detect_TwoCicle(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    # cv_show('Original_image', img)
